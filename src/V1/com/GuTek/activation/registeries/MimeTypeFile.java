...
public class MimeTypeFile {
    ...
    public MimeTypeFile(final InputStream is) throws IOException {
        this.fname = null;
        this.type_hash = new Hashtable();
        this.parse(new BufferedReader(new InputStreamReader(is, charsetName: "iso-8859-1")));
    }
    ...
    private void parse(final BufferedReader buf_reader) throws IOException {
        ...
        while ((line = buf_reader.readLine()) != null) {
            ...
            if (prev.length() > 0 && prev.charAt(end - 1) == '\\') {
                prev = prev.substring(beginIndex:0, end - 1);
            }
            else {
    ...
    private void parseEntry(String line) {
        ...
        if (line.indexOf(ch:61) > 0) {
            final LineTokenizer lt = new LineTokenizer(line);
            while (lt.hasMoreTokens()) {
                final String name = lt.nextToken();
                String value = null;
                if (lt.hasMoreTokens() && lt.nextToken().equals(anObject:"=") && lt.hasMoreTokens()) {
                    value = lt.nextToken();
                }
                if (value == null) {
                ...
                if (name.equals(anObject:"type")) {
                    mime_type = value;
                }
                else {
                    if (!name.equals(anObject:"exts")) {
                        continue;
                    }
